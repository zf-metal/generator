...
class ViewHelperController extends AbstractActionController {
    ...
    public function mainAction() {
        ...
        $query = $this->getEm()->createQueryBuilder()
                ->select('u')
                ->from('ZfMetal\Generator\Entity\ViewHelper', 'u')
                ->where("u.module = :moduleId")
                ->setParameter("moduleId", $moduleId);
        $source = new \ZfMetal\Datagrid\Source\DoctrineSource($this->getEm(), "ZfMetal\Generator\Entity\ViewHelper", $query);
        $this->grid->setSource($source);

        ##################################################
        $this->grid->addExtraColumn("generator", "<a class='btn btn-danger btn-xs glyphicon glyphicon-play' onclick='MetalViewHelper.generatorAction({{id}},\"{{name}}\")' ></a>", "right", false);
